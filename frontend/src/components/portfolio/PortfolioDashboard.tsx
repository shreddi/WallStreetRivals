...
export default function PortfolioDashboard({ originalPortfolio }: PortfolioDashboardProps) {
    // **State**
    const portfolioNum = 1;
    const [quantity, setQuantity] = useState<number>(0);
    const [portfolio, setPortfolio] = useState<Portfolio>(originalPortfolio);
    ...
    const formatPrice = (priceString: string): string => {
        return parseFloat(priceString).toLocaleString("en-US", {
    ...
    const rows = portfolio.holdings.reverse().map((holding) => (
        <Table.Tr key={holding.stock_data?.ticker || ""}>
            ...
            <Table.Td>
                {new Date(holding.time_updated || "").toLocaleString()}
            </Table.Td>
            <Table.Td>
                <Button
                    size="xs"
                    color="gray"
                    onClick={() => sellHolding(holding)}
                >
                    SELL
                </Button>
            </Table.Td>
                ...
                <Stack gap="0px">
                    ...
                    >
                        ...
                        <Title order={1} c="white">
                            ...
                            {formatPrice(
                                (
                                    parseFloat(portfolio.cash) +
                                    parseFloat(portfolio.holdings_total)
                                ).toString()
                            ).toLocaleString()}
                        </Title>
                    </Flex>
